...
class KeyboardReader:
	def __init__(
		...
		self.start_time: float = reactor.monotonic()
	...
	def connect(self, eventtime) -> float:
		...
		self.output(f"Loaded {message_count} commands ({version} / {build_versions})")
		msg = " ".join([f"{k}={v}" for k, v in msgparser.get_constants().items()])
		self.output(f"MCU config: {msg}")
	...
	def handle_default(self, params) -> None:
		...
		self.output(f"{tdiff:07.3f}: {msg}")

	def handle_output(self, params) -> None:
		tdiff = params["#receive_time"] - self.start_time
		self.output(f"{tdiff:07.3f}: {params['#name']}: {params['#msg']}")
	...
	def translate(self, line, eventtime) -> Optional[str]:
