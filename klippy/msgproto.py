...
class MessageParser:
	...
	def get_constant(
		...
		name,
		default=sentinel,
		parser: Union[Type[str], Type[int], Type[float]] = str
	) -> Union[str, int, float]:
		if name not in self.config:
			if default is not self.sentinel:
				return default
			self._error(f"Firmware constant '{name}' not found")
		try:
			value = parser(self.config[name])
		except:
			self._error(
				f"Unable to parse firmware constant {name}: {self.config[name]}"
			)
		return value

	def get_constant_float(self, name, default=sentinel) -> float:
		return self.get_constant(name, default, parser=float)

	def get_constant_int(self, name, default=sentinel) -> int:
