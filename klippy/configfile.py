...
class ConfigWrapper:
	...
	def __init__(
		...
		access_tracking: Dict[Tuple[str, str], str],
		section: str,
	):
	...
	def _get_wrapper(
		...
		option: str,
		default,
		minval: Union[int, float, None] = None,
		maxval: Union[int, float, None] = None,
		above: Union[int, float, None] = None,
		below: Union[int, float, None] = None,
	...
	def get(self, option: str, default=sentinel, note_valid: bool = True):
		return self._get_wrapper(
			self.fileconfig.get, option, default, note_valid=note_valid
		)

	def getint(
		self, option: str, default=sentinel, minval=None, maxval=None, note_valid: bool = True
	) -> int:
		return self._get_wrapper(
	...
	def getboolean(self, option: str, default=sentinel, note_valid: bool = True):
		return self._get_wrapper(
			self.fileconfig.getboolean, option, default, note_valid=note_valid
		)

	def getchoice(self, option: str, choices, default=sentinel, note_valid: bool = True):
	...
	def getlists(
		...
		def lparser(value, pos) -> Tuple[str, ...]:
		...
		def fcparser(section, option) -> Tuple[str, ...]:
	...
	def getlist(self, option: str, default=sentinel, sep=",", count=None, note_valid: bool = True):
	...
	def get_prefix_options(self, prefix) -> List[str]:
		return [
			o for o in self.fileconfig.options(self.section) if o.startswith(prefix)
		]

	def deprecate(self, option, value=None) -> None:
		if not self.fileconfig.has_option(self.section, option):
			return
		if value is None:
			...
			msg = (
				f"Value '{value}' in option '{option}' in "
				f" section '{self.section}' is deprecated."
			)
...
class PrinterConfig:
	def __init__(self, printer: Printer):
		self.printer = printer
		self.autosave: Optional[ConfigWrapper] = None
		self.deprecated = {}
		...
		self.save_config_pending: bool = False
	...
	def _read_config_file(self, filename: Union[Path, str]) -> str:
	...
	def _parse_config_buffer(
		self, buffer: List[str], filename: str, fileconfig: configparser.RawConfigParser
	) -> None:
		if not buffer:
			return
		data = "\n".join(buffer)
	...
	def _build_config_wrapper(self, data: str, filename: str) -> ConfigWrapper:
		if sys.version_info.major >= 3:
			fileconfig = configparser.RawConfigParser(
	...
	def read_config(self, filename) -> ConfigWrapper:
	...
	def check_unused_options(self, config) -> None:
	...
	def deprecate(self, section, option, value=None, msg=None) -> None:
		self.deprecated[(section, option, value)] = msg

	def _build_status(self, config: ConfigWrapper) -> None:
	...
	def _disallow_include_conflicts(
		self, regular_data: str, cfgname: str, gcode
	) -> None:
		...
		for section in self.autosave.fileconfig.sections():
			for option in self.autosave.fileconfig.options(section):
				if config.fileconfig.has_option(section, option):
					msg = (
						f"SAVE_CONFIG section '{section}' option '{option}' "
	...
	def cmd_SAVE_CONFIG(self, gcmd):
		...
		logging.info(f"SAVE_CONFIG to '{cfgname}' (backup in '{backup_name}')")
		try:
			f = open(temp_name, "w")
			f.write(data)
			f.close()
