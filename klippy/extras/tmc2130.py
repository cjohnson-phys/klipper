...
class TMCCurrentHelper:
	def __init__(self, config: ConfigWrapper, mcu_tmc):
...
class MCU_TMC_SPI_chain:
	def __init__(self, config: ConfigWrapper, chain_len=1):
		self.printer = config.get_printer()
...
class MCU_TMC_SPI:
	def __init__(self, config: ConfigWrapper, name_to_reg, fields, tmc_frequency):
		self.printer = config.get_printer()
		self.name = config.get_name().split()[-1]
...
def load_config_prefix(config: ConfigWrapper) -> TMC2130:
