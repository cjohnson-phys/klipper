...
class EddyCalibration:
	...
	def do_calibration_moves(self, move_speed):
		...
		max_z = 4
		samp_dist = 0.050
		num_steps = int(max_z / samp_dist + 0.5) + 1
...
class EddyEndstopWrapper:
	def __init__(self, config: ConfigWrapper, sensor_helper, calibration):
...
def load_config_prefix(config: ConfigWrapper) -> PrinterEddyProbe:
