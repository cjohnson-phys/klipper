...
class SoftwareI2C:
	def __init__(self, config: ConfigWrapper, addr):
		self.addr = addr << 1
		...
		for data in msg:
			...
			for i in range(8):
				...
				if sda_last != sda_next:
					sda_last = sda_next
					send([self.sda_oid, sda_last], minclock=minclock, reqclock=reqclock)
...
def load_config_prefix(config: ConfigWrapper) -> mcp4018:
